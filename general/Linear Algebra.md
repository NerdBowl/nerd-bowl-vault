...
1. Let $\mathbf{x}$ be any vector in $\mathbb{R}^n$. We can write $\mathbf{x}$ as a linear combination of the standard basis vectors: $$\mathbf{x} = \begin{bmatrix} x_1 \\ \vdots \\ x_n \end{bmatrix} = x_1\begin{bmatrix} 1 \\ \vdots \\ 0 \end{bmatrix} + \dots + x_n\begin{bmatrix} 0 \\ \vdots \\ 1 \end{bmatrix} = x_1\mathbf{e}_1 + \dots + x_n\mathbf{e}_n$$
